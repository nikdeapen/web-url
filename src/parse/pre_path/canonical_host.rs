...
impl CanonicalHost {
    ...
    ///
    /// The longest is an IPv6 address with an embedded IPv4 address and the '[]' brackets:
...
impl CanonicalHost {
    ...
    /// display, so it is lowercase and its zero groups are elided.
    pub fn new(ip: IPAddress) -> Self {
        ...
        debug_assert!(result.is_ok(), "the canonical host of '{}' is too long", ip);
